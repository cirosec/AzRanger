...
namespace AzRanger.AzScanner
{
    class ProvisionAPICollector : AbstractCollector
    {
        ...
        public async Task<MsolCompanyInformation> GetMsolCompanyInformation()
        {
            ...
            catch (Exception ex) { logger.Debug("ProvisionAPICollector.GetMsolCompanyInformation: GetCompanyInformationResponse failed: {0}", ex.Message); }
            if (response != null)
            {
        ...
        private async Task<String> PostToProvisioninApi(string command, string requestElement)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    catch (Exception ex) { logger.Debug("ProvisionAPICollector.PostToProvisioninApi: Reading error response body failed: {0}", ex.Message); }
                }
                return null;
            }
            finally
        ...
        public async Task<SharePointInformation> GetSharepointInformation()
        {
            ...
            if (getCompanyInformationResponse != null)
            {
                ...
                catch (Exception ex) { logger.Debug("ProvisionAPICollector.GetSharepointInformation: Traversing ServiceInformation failed: {0}", ex.Message); }
            }
            ...
            if (response != null)
            {
                ...
                {
                    logger.Debug("ProvisionApiScanner.GetSharepointInformation: Read XML failed");
                    logger.Debug(ex.Message);
                }
            }
            else
            {
                logger.Debug("ProvisionApiScanner.GetSharepointInformation: Post to provision API failed");
            }
