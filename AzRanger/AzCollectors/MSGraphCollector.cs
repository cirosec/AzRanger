...
namespace AzRanger.AzScanner
{
    ...
    public class MSGraphCollector : AbstractCollector
    {
        ...
        public const String ActivityBasedTimeoutPolicies = "/v1.0/policies/activityBasedTimeoutPolicies";
        public const String LegacyPolicies = "/beta/legacy/policies";
        public const String GroupSettings = "/v1.0/groupSettings";
        ...
        public Task<List<ActivityBasedTimeoutPolicy>> GetActivityBasedTimeoutPolicies()
        {
            return GetAllOf<ActivityBasedTimeoutPolicy>(ActivityBasedTimeoutPolicies);
        }
        ...
        public async Task<Dictionary<Guid, Device>> GetAllDevices()
        {
            ...
            return allDevices.ToDictionary(d => d.id);
        }
        public async Task<Dictionary<Guid, User>> GetAllUsers(bool hasP1License)
        {
            List<User> allUsers;
            if (hasP1License)
            {
            ...
            Dictionary<Guid, User> resultingUsers = allUsers.ToDictionary(u => u.id);

            List<UserRegistrationDetail> userRegistrationDetails = await GetUserRegistrationDetail("?$Filter=UserType eq 'Member'");
            foreach (UserRegistrationDetail detail in userRegistrationDetails)
            {
                if (resultingUsers.ContainsKey(Guid.Parse(detail.id)))
                {
                    ...
                    resultingUsers[Guid.Parse(detail.id)].userRegistrationDetail = detail;
                }
            }
        ...
        public async Task<Dictionary<Guid, User>> GetAllGuests(bool hasP1Licnse)
        {
            ...
            Dictionary<Guid, User> result = allUsers.ToDictionary(u => u.id);

            List<UserRegistrationDetail> userRegistrationDetails = await GetUserRegistrationDetail("?$Filter=UserType eq 'Guest'");
            foreach (UserRegistrationDetail detail in userRegistrationDetails)
            {
                if (result.ContainsKey(Guid.Parse(detail.id)))
                {
                    result[Guid.Parse(detail.id)].isMFAEnabled = detail.isMfaCapable;
                    result[Guid.Parse(detail.id)].userRegistrationDetail = detail;
                }
            }
        ...
        public async Task<List<AzurePrincipal>> GetAllMemberOf(Guid groupId)
        {
            ...
            {
                AzurePrincipal p = ToAzurePrincipal(member);
                if (p != null && p.PrincipalType != AzurePrincipalType.Group && !Result.Contains(p))
                {
                    Result.Add(p);
                }
                else if (p == null)
        ...
        internal async Task<List<AzurePrincipal>> GetAllRoleMember(Guid roleId)
        {
            ...
            foreach (IDTypeResponse member in All)
            {
                if (member.odatatype == "#microsoft.graph.group")
                {
                    ...
                    {
                        AzurePrincipal p = ToAzurePrincipal(user);
                        if (p != null && p.PrincipalType != AzurePrincipalType.Group && !Result.Contains(p))
                        {
                            Result.Add(p);
                        }
        ...
        public async Task<Dictionary<Guid, ConditionalAccessPolicy>> GetAllCondtionalAccessPolicies()
        {
            ...
            return policies.ToDictionary(p => Guid.Parse(p.id));
        }
        ...
        public async Task<Dictionary<Guid, ServicePrincipal>> GetAllServicePrincipals()
        {
            ...
            Dictionary<Guid, ServicePrincipal> result = allAppsWithOwner.ToDictionary(sp => sp.id);
        ...
        {
            List<Group> allGroups = await GetAllOf<Group>(GroupsBeta);
        ...
        internal async Task<List<AzurePrincipal>> GetAllGroupMemberTransitiv(Guid groupId)
        {
            ...
            {
                AzurePrincipal p = ToAzurePrincipal(principal);
                if (p != null && p.PrincipalType != AzurePrincipalType.Group && !result.Contains(p))
                {
                    result.Add(p);
                }
        ...
        private async Task GetAllGroupMemberTransitivNonBlocking(Guid groupId, List<AzurePrincipal> members)
        {
            ...
            {
                AzurePrincipal p = ToAzurePrincipal(principal);
                if (p != null && p.PrincipalType != AzurePrincipalType.Group && !members.Contains(p))
                {
                    members.Add(p);
                }
            }
        ...
        private Task<List<UserRegistrationDetail>> GetUserRegistrationDetail(String filter)
        {
            return GetAllOf<UserRegistrationDetail>(UserRegistrationDetails, filter);
        }

        internal Task<List<LegacyPolicy>> GetLegacyPolicies()
        {
            return GetAllOf<LegacyPolicy>(LegacyPolicies);
        }

        internal Task<List<GroupSetting>> GetGroupSettings()
        {
            return GetAllOf<GroupSetting>(GroupSettings);
        }

        private static AzurePrincipal ToAzurePrincipal(IDTypeResponse member)
        {
            switch (member.odatatype)
            {
                case "#microsoft.graph.user":
                    return new AzurePrincipal(member.id, AzurePrincipalType.User);
                case "#microsoft.graph.servicePrincipal":
                    return new AzurePrincipal(member.id, AzurePrincipalType.ServicePrincipal);
                case "#microsoft.graph.group":
                    return new AzurePrincipal(member.id, AzurePrincipalType.Group);
                default:
                    return null;
            }
        }
